...
description: |
  Generating claims data that can be used for loss reserving research and backtesting.
authors:
...
Today we're excited to announce the [simulationmachine](https://github.com/kasaai/simulationmachine) package, based on the paper *An Individual Claims History Simulation Machine*[@gabrielli2018individual]. If you're a practitioner or researcher looking for realistic data for testing reserving or claims forecasting models, you may find it interesting.
...
First, specify the desired characteristics of the simulation:
...
Here, we are noting that we would like 
...
In the output data frame, each row corresponds to a snapshot in time of a claim. At this point, the dataset is ready for use in a claims level exercise. However, if you're looking for aggregate triangle data, that can be easily done with a few transformations. Say we're interested in a cumulative paid loss triangle as of calendar year 1999 (so it'll fit in the margins of this blog post), we can use the following snippet:
